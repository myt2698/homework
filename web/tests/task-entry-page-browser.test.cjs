...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('#taskEntryEstimate').inputValue(), '15');
    await page.screenshot({ path: path.join(output, 'empty-390.png'), fullPage: true });
    for (let i = 1; i <= 20; i++) {
      if (i === 2) await page.locator('#taskEntryEstimate').selectOption('30');
      await page.locator('#taskDraft').fill(`第${i}项作业`);
      const before = await page.locator('#taskDraft').boundingBox();
      if (i === 2) await page.locator('#taskDraft').press('Enter');
      else await page.locator('#addTasksButton').click();
    ...
    assert.equal(await page.locator('#taskEntryEstimate').inputValue(), '30', 'back navigation retains the draft estimate');
    ...
    for (const [width, height] of [[320, 740], [390, 844], [1100, 900], [390, 430]]) {
      ...
      const estimate = await page.locator('#taskEntryPage .task-entry-estimate').boundingBox();
      const add = await page.locator('#addTasksButton').boundingBox();
      assert(estimate.x + estimate.width <= add.x && Math.abs(estimate.y - add.y) < 1, 'estimate sits immediately left of the add button at every width');
      assert(estimate.y + estimate.height <= height, 'estimate remains visible with the keyboard open');
    ...
    assert.equal(saved.tasks[0].estimatedMinutes, 15);
    assert(saved.tasks.slice(1).every(task => task.estimatedMinutes === 30), 'click and Enter both persist the selected estimate');

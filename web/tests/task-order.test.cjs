...
const settingsMarkup = html.split('id="settingsPage"')[1].split('id="alarmSettingsPage"')[0];
assert(settingsMarkup.includes('id="openAlarmSettingsButton"') && settingsMarkup.includes('id="openBackupSettingsButton"'), 'settings keeps compact reminder and backup entries');
assert(!/id="(?:recordAlarmButton|exportDataButton|importDataButton)"/.test(settingsMarkup), 'settings no longer embeds reminder or backup controls');
...
const nativeSettings = androidSource.split('private View buildSettingsPage(')[1].split('private View buildSettingsDetailEntry(')[0];
assert(nativeSettings.includes('showSettingsDetailPage(true)') && nativeSettings.includes('showSettingsDetailPage(false)'), 'native settings has both detail entries');
assert(!nativeSettings.includes('toggleBreakAlarmRecording()') && !nativeSettings.includes('launchBackupExport()'), 'native settings does not embed detail operations');
...
const nativeRemainingLabel = androidSource.slice(androidSource.indexOf('private SpannableString remainingTimeLabel(')).split('private void updateTaskRemainingTime(')[0];
...
assert(nativeRemainingLabel.includes('new StyleSpan(Typeface.BOLD)') && nativeRemainingLabel.includes('new ForegroundColorSpan(color)') && nativeRemainingLabel.includes('return remainingTimeLabel(minutes, Color.rgb(52,95,158))'), 'native overview keeps its bold blue highlight while focus can use a neutral color');

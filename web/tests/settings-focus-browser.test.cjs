...
(async () => {
  ...
  try {
    ...
    const normal = await dimensions(['subjectPickerButton', 'taskDraft', 'taskEntryEstimate', 'addTasksButton']);
    await page.evaluate(() => {
    ...
    const holiday = await dimensions(['holidaySubject', 'holidayTaskTitle', 'holidayTaskMinutes']);
    assert.deepEqual(holiday, normal.slice(0, 3), 'ordinary and holiday subject, input and estimate dimensions match');
    ...
    assert.equal(await page.locator('#holidayTaskMinutes').inputValue(), '15');
